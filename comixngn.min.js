...
n.dia/30,n.ctx.strokeStyle=f,n.ctx.stroke();n.ctx.restore();v?L(g):n.ctx.clearRect(0,0,300,l[1].height)}},k=(b,f)=>{if(null===b||void 0===b)b={x:0,y:0};else if(isNaN(b)){if(null===b.y||void 0===b.y)b.y=0;if(null===b.x||void 0===b.x)b.x=0}else b={x:0,y:b};if(null===f||void 0===f||0>=f)f=400;0>b.y&&(b.y=window.innerHeight||H.clientHeight||K.clientHeight);0>b.x&&(b.x=window.innerWidth||H.clientWidth||K.clientWidth);b={x:void 0!==window.pageXOffset?b.x-window.pageXOffset:b.x-H.scrollLeft,y:void 0!==window.pageYOffset?
b.y-window.pageYOffset:b.y-H.scrollTop};if(b=={x:0,y:0})return b;let h=function(t,u,z){window.scrollBy(Math.floor(t.x)/u,Math.floor(t.y)/u);z+1<5*u&&(B=window.setTimeout(h,5,t,u,z+1))};B=window.setTimeout(h,5,b,Math.floor(f/5),0);return b},p=b=>{const {width:f,height:h}=b;U.clearRect(0,0,l[1].width,l[1].height);D.run("slidn");if(w.sz){let {w:t,h:u,s2w:z,s2h:M,s2g:N,s2l:V}=w.sz,O=1,P=1,Q=t||f,R=u||h;if(N||V)z=N?t>=u:u>t,M=!z;z&&(O=Q/f);M&&(P=R/h);l[1].width=Q*P;l[1].height=R*O}else l[1].width=f,l[1].height=
h;n&&(l[0].height=l[1].height);U.drawImage(b,0,0,...[l[1].width,l[1].height]);v=!1;W&&k();D.run("slidd")},r=b=>{var f=[b.width,b.height];let h=m[b.virID],t=u=>{h&&(h.ib=u);b.master&&p(u)};if(b.ui)h=b;else{X.clearRect(0,0,l[2].width,l[2].height);l[2].width=f[0];l[2].height=f[1];X.drawImage(b,0,0);try{h.ui=X.getImageData(0,0,...f)}catch(u){return console.log("Cannot get imageData from cross-origin"),f=l[2].transferToImageBitmap(),t(f)}}Y?(f=Y.createKernel(c.shader||function(u){var z=4*(this.thread.x+
this.constants.w*(this.constants.h-this.thread.y));this.color(u[z]/256,u[z+1]/256,u[z+2]/256,u[z+3]/256)}).setConstants({w:f[0],h:f[1]}).setOutput(f).setGraphical(!0),f.canvas.width=1,f.canvas.height=1,f.maxTexSize=[1,1],f(h.ui.data),createImageBitmap(f.canvas).then(t)):(f=c.pixelfn,createImageBitmap(f?f(h.ui):h.ui).then(t))},q=b=>{if(m.length){v=!0;L(g);D.run("start");b=Math.min(Math.max(0,b),m.length-1);m[b].ib?m[b].st!=m.shaderTime?r(m[b]):p(m[b].ib):(U.clearRect(0,0,l[1].width,l[1].height),S.virID=
b,S.src=w.dir+m[b].s);E=b;var f,h=0;for(f=b-1;h<w.irb&&0<=f;f--)if(!m[f].ib)if(m[f].st!=m.shaderTime)r(m[f]);else{var t=h,u=w.dir+m[f].s;T[t].virID=f;T[t].src=u;h++}for(f=b+1;h<w.itb&&f<m.length;f++)m[f].ib||(m[f].st!=m.shaderTime?r(m[f]):(b=h,t=w.dir+m[f].s,T[b].virID=f,T[b].src=t,h++))}},x=(b,f)=>{if(b)for(var h in f)b.hasOwnProperty(h)&&(b[h]=f[h])},I=b=>Array(b).fill(1).map((f,h)=>{f=new Image;f.unique=h;f.virID=-1;f.addEventListener("load",r.bind(null,f));return f}),A=b=>{let f=Date.now();b=
("string"==typeof b?b.split(" "):b).map((h,t,u)=>({s:h,st:f,d:t?t==u.length-1?1:0:-1}));b.shaderTime=f;return b},J=()=>{try{jQuery.fn.direction=function(b,f){return this.each(function(){f.anchor=$(this);d(b,f)})}}catch(b){console.log(b)}},F=document,K=F.body,H=F.documentElement,G=parseInt,L=window.requestAnimationFrame;let y=c.overwrite||0,C=c.anchor||K,m=A(a),v=!0,B=-1,E=-1,l={0:c.disableSpin?null:F.createElement("canvas"),1:F.createElement("canvas"),2:new OffscreenCanvas(e[0],e[1])},n=l[0]?{ctx:l[0].getContext("2d"),
clr:c.color||"#373737",str:Date.now(),lne:c.lines||16,rte:c.rate||1E3/30,dia:c.diameter||250,lbk:c.loaderback||"#FFF",dat:Date.now(),sav:[]}:null,w={dir:c.dir||"",irb:c.imgprebuffer||5,itb:c.imgpostbuffer||5,bck:c.back||"#FFF",sz:c.size||0},T=I(w.itb+w.irb),S=I(1)[0],W=!0,U=l[1].getContext("2d"),X=l[2].getContext("2d"),D={run:b=>{for(var f=0;f<D[b].length;f++)D[b][f]()},start:[],slidn:[],slidd:[]},Y;window.jQuery&&J();this.canvi=l;this.cb=D;this.setupShaders=b=>{if(b.gpu){const f=new OffscreenCanvas(e[0],
e[1]),h=f.getContext("webgl2",{premultipliedAlpha:!1});Y=new b.gpu({canvas:f,webGl:h})}b.shader&&(c.shader=b.shader);b.pixelfn&&(c.pixelfn=b.pixelfn)};this.cnl=()=>{window.clearTimeout(B)};this.swap=(b,f,h)=>{m=Array.isArray(b)?A(b):m;f&&(x(n,f),x(w,f));this.go(h||0)};this.count=()=>m.length;this.current=()=>E;this.callback=(b,f,h,t)=>{if(null===b||void 0===b)return D.slidn;b={"-1":D.start,0:D.slidn,1:D.slidd}[h||0];if(t)return b.splice(h||b.length-1,1);if(null===f||void 0===f)return null===h||void 0===
h?b:b[h];null===h||void 0===h?b.push(f):b[h]=f;return 1};this.go=b=>{b=null===b||void 0===b?0:G(b,10);b=isNaN(b)?0:b;q(Math.floor(Math.max(0,Math.min(m.length-1,b))));return b};this.prev=()=>{var b=E-1;0<=b&&q(b);return b};this.next=()=>{var b=E+1;b<m.length&&q(b);return b};this.frst=()=>{0<=E&&q(0);return 0};this.last=()=>{q(m.length-1);return m.length-1};this.rand=()=>{var b=Math.floor(Math.random()*(m.length-1));q(b);return b};this.data=b=>{b=null===b||void 0===b?E:G(b,10);return isNaN(b)?m[E]:
m[Math.floor(Math.max(0,Math.min(m.length-1,b)))]};this.scroll=b=>{null!==b&&void 0!==b&&(W=b);return W};this.scrollTo=(b,f)=>k(b,f);this.resize=(b,f=!0,h=!1)=>{w.sz=h?{...b,w:e[0],h:e[1]}:b;f&&this.go(E);return b};n&&(l[0].height=480,l[0].style.background=n.lbk,l[0].style.paddingLeft="170px",l[0].style.zIndex=0,l[0].style.position="absolute",C.appendChild(l[0]));if(w.sz){let {w:b,h:f,width:h,height:t,sh:u,sw:z,sg:M,sl:N,s2h:V,s2w:O,s2g:P,s2l:Q,scaleToHeight:R,scaleToWidth:Z,scaleToGreater:aa,scaleToLesser:ba}=
w.sz;w.sz={w:b||h,h:f||t,s2w:z||O||Z,s2h:u||V||R,s2g:M||P||aa,s2l:N||Q||ba}}S.master=!0;S.unique=-1;q(w.startpage||y);l[1].height=e[0];l[1].width=e[1];l[1].background=w.bck;l[1].style.zIndex=1;l[1].style.position="relative";C.appendChild(l[1])}var module$lib$directionx={};module$lib$directionx.default=direction$$module$lib$directionx;function pegasus$$module$lib$pegasus_min(a,c,e){return e=new XMLHttpRequest,e.open("GET",a),a=[],pegasus$$module$lib$pegasus_min.timeout&&(e.timeout=pegasus$$module$lib$pegasus_min.timeout),e.ontimeout=function(g){c=g},e.onreadystatechange=e.then=function(g,k,p,r){if(g&&g.call&&(a=[,g,k]),c&&a[2]&&a[2](c,e),4==e.readyState&&(p=a[0|e.status/200])){try{r=JSON.parse(e.responseText)}catch(q){r=null}p(r,e)}},e.send(),e}var module$lib$pegasus_min={};module$lib$pegasus_min.default=pegasus$$module$lib$pegasus_min;const Path$$module$lib$path_min={version:"0.8.4",map:function(a){return Path$$module$lib$path_min.routes.defined.hasOwnProperty(a)?Path$$module$lib$path_min.routes.defined[a]:new Path$$module$lib$path_min.core.route(a)},root:function(a){Path$$module$lib$path_min.routes.root=a},rescue:function(a){Path$$module$lib$path_min.routes.rescue=a},history:{initial:{},pushState:function(a,c,e){Path$$module$lib$path_min.history.supported?Path$$module$lib$path_min.dispatch(e)&&history.pushState(a,c,e):Path$$module$lib$path_min.history.fallback&&
...
{},g=null,k,p;for(g in Path$$module$lib$path_min.routes.defined)if(null!==g&&void 0!==g){g=Path$$module$lib$path_min.routes.defined[g];var r=g.partition();for(p=0;p<r.length;p++){var q=r[p];var x=a;if(0<q.search(/:/))for(k=0;k<q.split("/").length;k++)k<x.split("/").length&&":"===q.split("/")[k].charAt(0)&&(e[q.split("/")[k].replace(/:/,"")]=x.split("/")[k],x=x.replace(x.split("/")[k],q.split("/")[k]));if(q===x)return c&&(g.params=e),g}}return null},dispatch:function(a){if(Path$$module$lib$path_min.routes.current!==
...
{config:g,loading:k}=a,{dir:p,imgprebuffer:r,imgpostbuffer:q,startPage:x}=g,{diameter:I,lines:A,rate:J}=k;return Object.assign({overwrite:x,dir:p,imgprebuffer:r,imgpostbuffer:q,diameter:I,lines:A,rate:J,back:g.back?g.back.toString():void 0,loaderback:k.back?k.back.toString():void 0,color:k.color?k.color.toString():void 0},e)}initializeDisplay(){var {shadow:a}=this;const c=this._schema?this._schema.exportPages():[],e=this._schema?this.convertToDirectionSetting():{};a=new direction$$module$lib$directionx(c,
Object.assign(Object.assign({},e),{anchor:a}));this.defineMethods(a);console.log("Intialize Display");this._active=!0;(a=this.getAttribute("controller"))?pegasus$$module$lib$pegasus_min(a).then(this.initializeControls.bind(this)):this.initializeControls()}initializeControls(a){this.controller=new CmxCtrl$$module$_compile$cmxctrl(this,a);this.insertAdjacentElement("afterend",this.controller)}defineMethods(a){let c=Object.assign({},this._core.setting),e=k=>0,g=k=>0;this._schema&&(e=this._schema.pageToChapter,
g=this._schema.chapterToPage,c=Object.assign(Object.assign({},c),this._schema));this.rand=a.rand;this.go=a.go;this.prev=a.prev;this.next=a.next;this.frst=a.frst;this.last=a.last;this.ch_go=k=>e(this.go(e(k)));this.ch_prev=()=>e(this.go(g(this.ch_current()-1)));this.ch_next=()=>e(this.go(g(this.ch_current()+1)));this.ch_frst=()=>e(this.go(g(0)));this.ch_last=()=>e(this.go(g(this._schema?this._schema.chapters.length:0)));this.update=()=>{const k=a.swap,p=this._schema?this._schema.exportPages():[],r=
this._schema?this.convertToDirectionSetting():{};k(p,Object.assign(Object.assign({},r),{anchor:this.shadow}));a.setupShaders(r)};this.resize=a.resize;this.current=a.current;this.ch_current=()=>{e(this.current())};this.rawData=a.data;this.pg_data=k=>{if(this._schema&&this._schema.pages.length)return this._schema.pages[k||this.current()||0]};this.ch_data=k=>{if(this._schema&&this._schema.chapters.length)return this._schema.chapters[k||this.ch_current()||0]};a.callback(1,()=>{var k=this.current(),p=
this._schema.pages[k];if(localStorage&&c.pageSave){const {_cid:q,_uid:x}=this;localStorage.setItem(`${q}|${x}|current`,k.toString())}if(c.pagePush){var r="";if(c.config.format){const {format:q,chapterStartAt:x,pageStartAt:I}=c.config,A=p.release,J=p.title,F=p.url[0],K=k+I,H=this._schema.pageToChapter(k)+x,G=this.ch_data()?this.ch_data().title:"",L="january febuary march april may june july august september october november december".split(" ");k=(y,C)=>{if(!y)return"";const m=y===y.toUpperCase();
y=y.toLowerCase();var v=B=>()=>{if(!A)throw Error(`Date required for format of ${q}`);return B()};v={y:v(()=>{let B=A.getFullYear().toString();return 3>C?B.slice(2):B}),m:v(()=>A.getMonth().toString()),d:v(()=>A.getDay().toString()),h:v(()=>A.getHours().toString()),u:v(()=>A.getMinutes().toString()),s:v(()=>A.getSeconds().toString()),n:v(()=>{C=0;let B=L[A.getMonth()];return m?B:B.slice(0,3)}),c:()=>{C=0;return!m&&G?G:H.toString()},p:()=>{C=0;return!m&&J?J:K.toString()},f:()=>{C=0;return F.slice(F.lastIndexOf("/")+
1)},"":()=>""};v=(v[y]||v[""])();return v.length?v.padStart(C-v.length,"0"):y};p=[];r=[];for(let y=0;y<q.length;y++)if(p[0]!==q[y]){try{r.push(k(p[0],p.length))}catch(C){return console.log(C),""}p=[q[y]]}else p.push(q[y]);r.push(k(p[0],p.length));r=r.join("")}console.log(`Pushing state: ${r}`);history.pushState({},"",`#/${r}`)}})}exportSchema(){return JSON.stringify(this._schema)}changeId(a,c){const {_cid:e,_uid:g}=this,k=`${e}|${g}|current`;this[a]=c;a=`${e}|${g}|current`;if(c=localStorage.getItem(k))localStorage.removeItem(k),
